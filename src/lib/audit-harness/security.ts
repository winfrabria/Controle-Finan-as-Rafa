...
const SENSITIVE_TEXT_PATTERNS: Array<[RegExp, string]> = [
  ...
  [
    /\b(?:api[_ -]?key|authorization|private[_ -]?key|access[_ -]?token|refresh[_ -]?token|session[_ -]?token|auth[_ -]?token|client[_ -]?secret|csrf(?:[_ -]?token)?|jwt|nonce|token|secret|password|passwd|set[_ -]?cookie|cookie|session[_ -]?id)\s*[:=]\s*[^\r\n,;}]*/giu,
    "[REDACTED_CREDENTIAL]",
  ],
  [
